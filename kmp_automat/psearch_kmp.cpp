...
int findstr( automat & word_kmp, mutex & kmp_lock, vector<string> & fl_names, std::vector<string>::iterator & cur,   mutex & _cur_lock,bool & b, mutex & _lock_output){
	string s;
	while (b){
		_cur_lock.lock();
		string filename = *cur;
...
int main(int argc, char** argv){
	...
	if ( t_pth == 1){
		...
		findstr( word_kmp, kmp_lock, fl_names, cur, _cur_lock,b, _lock_output);
...
//        } 
    	...
    	for (int i = 1; i <= t_pth; i++){
			v_th.push_back(thread(findstr, ref(word_kmp),  ref(kmp_lock), ref(fl_names), ref(cur), ref(_cur_lock), ref(b), ref(_lock_output)));		
		}
		//wait for finish
